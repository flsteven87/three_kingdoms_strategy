...
export const hegemonyWeightKeys = {
  ...
  list: (seasonId: string) =>
    [...hegemonyWeightKeys.lists(), seasonId] as const,
  summaries: () => [...hegemonyWeightKeys.all, "summary"] as const,
  summary: (seasonId: string) =>
    [...hegemonyWeightKeys.summaries(), seasonId] as const,
 ...
 */
export function useHegemonyScoresPreview(
...
export function useInitializeHegemonyWeights() {
  ...
  return useMutation({
    mutationFn: (seasonId: string) =>
      apiClient.initializeHegemonyWeights(seasonId),
    onSettled: (_data, _error, seasonId) => {
      // Invalidate all related queries - ensures cache consistency
      ...
      queryClient.invalidateQueries({
        queryKey: hegemonyWeightKeys.previews(),
      });
    },
  });
}
...
export function useCreateHegemonyWeight() {
  ...
  return useMutation({
    ...
    onSettled: (_data, _error, variables) => {
      queryClient.invalidateQueries({
        queryKey: hegemonyWeightKeys.list(variables.seasonId),
      });
      queryClient.invalidateQueries({
        queryKey: hegemonyWeightKeys.summary(variables.seasonId),
      });
      queryClient.invalidateQueries({
        queryKey: hegemonyWeightKeys.previews(),
      });
...
export function useUpdateHegemonyWeight() {
  ...
  return useMutation({
    ...
    onMutate: async ({ weightId, data, seasonId }) => {
      ...
      const previousWeights = queryClient.getQueryData<
        HegemonyWeightWithSnapshot[]

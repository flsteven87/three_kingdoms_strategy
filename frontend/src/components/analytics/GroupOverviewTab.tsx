/**
 * GroupOverviewTab - Group overview with radar chart and comparisons
...
interface GroupOverviewTabProps {
  ...
  readonly groupParticipationRates: {
    overall: number;
    siege: number;
    battle: number;
  };
  readonly allGroupsParticipation: Map<
    string,
...
export function GroupOverviewTab({
  ...
  const radarData = [
    {
      metric: "貢獻",
      group: normalize(
        groupStats.avg_daily_contribution,
        allianceAverages.avg_daily_contribution,
      ),
      groupRaw: groupStats.avg_daily_contribution,
      alliance: 100,
      allianceRaw: allianceAverages.avg_daily_contribution,
      median: normalize(
        allianceAverages.median_daily_contribution,
        allianceAverages.avg_daily_contribution,
      ),
    ...
    },
    {
      metric: "戰功",
      group: normalize(
        groupStats.avg_daily_merit,
        allianceAverages.avg_daily_merit,
      ),
      groupRaw: groupStats.avg_daily_merit,
      alliance: 100,
      allianceRaw: allianceAverages.avg_daily_merit,
      median: normalize(
        allianceAverages.median_daily_merit,
        allianceAverages.avg_daily_merit,
      ),
    ...
    },
    {
      ...
      median: normalize(
        allianceAverages.median_power,
        allianceAverages.avg_power,
      ),
    ...
    },
    {
      metric: "助攻",
      group: normalize(
        groupStats.avg_daily_assist,
        allianceAverages.avg_daily_assist,
      ),
      groupRaw: groupStats.avg_daily_assist,
      alliance: 100,
      allianceRaw: allianceAverages.avg_daily_assist,
      median: normalize(
        allianceAverages.median_daily_assist,
        allianceAverages.avg_daily_assist,
      ),
    ...
    },
    {
      metric: "捐獻",
      group: normalize(
        groupStats.avg_daily_donation,
        allianceAverages.avg_daily_donation,
      ),
      groupRaw: groupStats.avg_daily_donation,
      alliance: 100,
      allianceRaw: allianceAverages.avg_daily_donation,
      median: normalize(
        allianceAverages.median_daily_donation,
        allianceAverages.avg_daily_donation,
      ),
  ...
  const meritDiff = calculatePercentDiff(
    groupStats.avg_daily_merit,
    allianceAverages.avg_daily_merit,
  );
  ...
  const chartData = allGroupsData.map((g) => ({
    name: g.name,
  ...
  const participationComparisonData = allGroupsData
    .map((g) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-primary/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold tabular-nums">
              {formatNumber(groupStats.avg_daily_contribution)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold tabular-nums">
              {formatNumber(groupStats.avg_daily_merit)}
            </div>
            <div className="flex items-center gap-1 mt-1">
              {meritDiff >= 0 ? (
                <TrendingUp className="h-3 w-3 text-primary" />
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold tabular-nums">
              {formatNumber(groupStats.avg_daily_assist)}
            </div>
            <div className="flex items-center gap-1 mt-1">
              {assistDiff >= 0 ? (
                <TrendingUp className="h-3 w-3 text-primary" />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              組別人日均表現 vs 同盟平均/中位數（100% = 同盟平均）
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer
              config={groupChartConfigs.capabilityRadar}
              ...
              <RadarChart data={radarData}>
                ...
                <PolarRadiusAxis
                  ...
                  domain={[
                    ...
                    Math.max(
                      150,
                      ...radarData.map((d) => Math.max(d.group, d.median)),
                    ),
                  ]}
                  tick={{ fontSize: 10 }}
                    ...
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        ...
                        <div className="text-sm">
                          ...
                          {formatNumberCompact(data.groupRaw)} ({data.group}%)
                        </div>
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer
              config={groupChartConfigs.meritBar}
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        ...
                        <div className="text-sm">
                          人日均戰功: {formatNumber(data.merit)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          平均排名: #{Math.round(data.avgRank)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          成員數: {data.memberCount}
                        </div>
                      </div>
                    );
                  }}
                ...
                <Bar dataKey="merit" radius={[0, 4, 4, 0]}>
                  {chartData.map((entry) => (
                    <Cell
                      key={entry.name}
                      fill={
                        entry.name === groupStats.group_name
                          ? "var(--primary)"
                          : "var(--muted)"
                      }
                    />
      ...
      <div className="grid gap-6 md:grid-cols-2">
              ...
              >
                ...
                <ChartTooltip
                  content={({ active, payload }) => {
                    ...
                    const data = payload[0]
                      .payload as (typeof participationByTypeData)[0];
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        <div className="font-medium">{data.type}活動</div>
                        <div className="text-sm">
                          參與率: {data.rate.toFixed(1)}%
                        </div>
                      </div>
                    );
                  }}
                ...
                <Bar dataKey="rate" radius={[4, 4, 0, 0]}>
                  {participationByTypeData.map((entry) => {
                    ...
                    return <Cell key={entry.type} fill={color} />;
                  })}
                </Bar>
              </BarChart>
            </ChartContainer>
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              <BarChart
                data={participationComparisonData}
                layout="vertical"
                margin={{ left: 0, right: 16 }}
              >
                <CartesianGrid
                ...
                <ChartTooltip
                  content={({ active, payload }) => {
                    if (!active || !payload?.length) return null;
                    const data = payload[0]
                      .payload as (typeof participationComparisonData)[0];
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        <div className="font-medium">{data.name}</div>
                        <div className="text-sm">
                          整體參與率: {data.overall.toFixed(1)}%
                        </div>
                        <div className="text-sm text-muted-foreground">
                          攻城: {data.siege.toFixed(1)}%
                        </div>
                        <div className="text-sm text-muted-foreground">
                          會戰: {data.battle.toFixed(1)}%
                        </div>
                      </div>
                    );
                  }}

/**
 * GroupMembersTab - Group members table with sorting
...
export function GroupMembersTab({
  ...
  const [sortBy, setSortBy] = useState<
    "rank" | "merit" | "assist" | "participation"
  >("rank");
                ...
                {sortedMembers.map((member) => (
                  <tr key={member.id} className="border-b last:border-0">
                    ...
                    <td className="py-2 px-2 text-right tabular-nums">
                      #{member.contribution_rank}
                    </td>
                    {viewMode === "latest" && (
                      <td className="py-2 px-2">
                        <div className="flex justify-end">
                          <RankChangeIndicator
                            change={member.rank_change}
                            showNewLabel={false}
                            size="sm"
                          />
                        </div>
                    ...
                    <td className="py-2 px-2 text-right tabular-nums">
                      {formatNumber(member.daily_merit)}
                    </td>
                    {viewMode === "latest" && (
                      <td className="py-2 px-2 text-right text-xs tabular-nums">
                        {member.merit_change === null ? (
                          ...
                          <span className="text-primary">
                            +{formatNumberCompact(member.merit_change)}
                          </span>
                        ) : member.merit_change < 0 ? (
                    ...
                    <td className="py-2 px-2 text-right tabular-nums">
                      {Math.round(member.daily_assist)}
                    </td>
                    <td className="py-2 px-2 text-right tabular-nums">
                      <span
                        className={
                          (memberParticipation.get(member.name) ?? 0) < 66.67

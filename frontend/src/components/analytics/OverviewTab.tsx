/**
 * OverviewTab - Alliance overview with KPI cards and trends
...
export function OverviewTab({ viewMode, data }: OverviewTabProps) {
  ...
  const dailyTrendData = expandPeriodsToDaily(
    trends,
    (p: AllianceTrendWithMedian) => ({
  ...
  const xAxisTicks = getPeriodBoundaryTicks(trends);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          subtitle={
            viewMode === "season" ? "賽季加權平均" : current_period.period_label
          }
          trend={
            summary.contribution_change_pct !== null
              ? { value: summary.contribution_change_pct, label: "% vs 前期" }
              : undefined
          }
          highlight
        />
        <KpiCard
          ...
          trend={
            summary.merit_change_pct !== null
              ? { value: summary.merit_change_pct, label: "% vs 前期" }
              : undefined
          }
          highlight
        />
        ...
        <KpiCard
          ...
          trend={
            summary.power_change_pct !== null
              ? { value: summary.power_change_pct, label: "% vs 前期" }
              : undefined
          }
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* Trend Line */}
            <ChartContainer
              config={allianceChartConfigs.trend}
                    ...
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        ...
                        <div className="text-sm">
                          平均: {formatNumber(d.contribution)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          中位數: {formatNumber(d.medianContribution)}
                        </div>
                      </div>
                    );
                  }}
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded-lg border bg-background p-2 shadow-sm">
                        <div className="font-medium">{d.dateLabel}</div>
                        <div className="text-sm">
                          平均: {formatNumber(d.merit)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          中位數: {formatNumber(d.medianMerit)}
                        </div>
                      </div>
                    );
                  }}

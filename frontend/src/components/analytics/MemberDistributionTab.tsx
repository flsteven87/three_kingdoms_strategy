...
import type { AllianceAnalyticsResponse, PerformerItem, AttentionItem, DistributionBin } from '@/types/analytics'
...
export function MemberDistributionTab({ viewMode, data }: MemberDistributionTabProps) {
                                    ...
                                    {SORT_COLUMNS.filter(col => !col.showOnlyLatest || viewMode === 'latest').map(col => (
                                        <th key={col.field} className="py-2 px-2">
                                            <button
                                                ...
                                                className={`flex items-center font-medium hover:text-primary transition-colors ${col.align === 'right' ? 'justify-end w-full' : ''
                                                    }`}
                                            >
                                                {col.label}
